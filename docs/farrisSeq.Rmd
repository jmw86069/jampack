...
```{r load_objects}
## Load the R objects
loadObject("farrisSalmonSE");
loadObject("farrisSalmonTxSE");
...
For the purpose of the R analysis methods described, the input data
...
R object `farrisSalmonSE`.

#### farrisSalmonSE format

The `farrisSalmonSE` object is a `SummarizedExperiment` with this content:
...
df2 <- kable_coloring(colData(farrisSalmonSE),
      colorSub=colorSub,
...
#### farrisSalmonTxSE format

The `farrisSalmonTxSE` object is a named list with content similar to that
...
the Gencode GTF file.
```{r, tx_table}
tx_df <- subset(as.data.frame(rowData(farrisSalmonTxSE)),
...
### Comparison with CA1 published data by Ainsley, Cajigas, Nakayama

4-way Venn diagram


### CB-only for CA1, CA2, CA3, DG

Per-sample correlation, centered across all CB samples.


#### Centered by Compartment (CB, DE)
