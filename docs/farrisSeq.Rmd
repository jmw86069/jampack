...
```{r genes_detected_CA1, fig.height=9, fig.width=6}
...
```{r sample_corr_cb_CH, fig.height=8, fig.width=10}
...
```{r corr_by_compartment_CB, fig.height=8, fig.width=10}
...
```{r, codon_usage}
## The Mouse codon usage file is available in the splicejam package
mouseCodonTxtFile <- system.file("extdata", "Mouse_codon_usage.txt", package="farrisdata")
#mouseCodonTxtFile <- "~/Projects/R-scripts/Mouse_codon_usage.txt";
codonDF <- codonUsage2df(mouseCodonTxtFile);

## Next load table of CAI data, from the seqinr package
data(caitab, package="seqinr");

## Use data in the same order as caitab
mouseCodonDF <- codonDF[match(rownames(caitab), rownames(codonDF)),,drop=FALSE];

## Translate codon to amino acid
transV <- sapply(rownames(mouseCodonDF), function(i){
   seqinr::translate(unlist(strsplit(i, "")));
});
## Split codons into a list by amino acid
transL <- split(names(transV), transV);
mouseCodonDF2 <- rbindList(lapply(names(transL), function(j){
   i <- transL[[j]];
   iDF <- mouseCodonDF[i,,drop=FALSE];
   iDF[,2] <- iDF[,2] / max(iDF[,2]);
   iDF[,"aa"] <- j;
   iDF;
}));
## Add number of codons per aa
mouseCodonDF2[,"degen"] <- lengths(transL)[mouseCodonDF2$aa];

## Change the frequency for stop codons to zero,
## since we are positing the frequency is proportional
## to readthrough efficiency.
mouseCodonDF2[mouseCodonDF2$aa %in% "*","frequency"] <- 0;
